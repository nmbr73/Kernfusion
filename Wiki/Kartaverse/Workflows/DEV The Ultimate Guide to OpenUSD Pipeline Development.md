...
Next, you need to download the PIXAR "[Kitchen_set](http://graphics.pixar.com/usd/downloads.html)" example USD scene.
...
![[filephp__fix14.png|Displaying the PIXAR USD "Kitchen_set.usd" example in usdview]]
...
After the build_usd.py script has been used to create usdview and the 3rd party OpenUSD plugins are compiled successfully, you will see output like this in the terminal:
...
With OpenUSD installed on your system the `/opt/r_usd/USD/bin/` folder will have the following USD tools you can run from the CLI (command-line):
